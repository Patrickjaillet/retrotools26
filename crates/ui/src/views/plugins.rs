            ...
            ui.horizontal(|ui| {
                ...
                ui.label(
                    state
                        ...
                        .unwrap_or_else(|| {
                            "No source folder (only needed by some plugins)".to_string()
                        }),
                );
            });
                ...
                ui.group(|ui| {
                    ...
                    if let Some(outcome) = state.plugin_last_outcomes.get(&id) {
                        match outcome {
                            Ok(summary) => {
                                ui.label(
                                    RichText::new(format!("Last run: {summary}"))
                                        .color(egui::Color32::from_rgb(76, 175, 80)),
                                );
                            }
                            Err(err) => {
                                ui.label(
                                    RichText::new(format!("Last run failed: {err}"))
