...
pub fn show(ui: &mut Ui, state: &AppState) {
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            let games_total: usize = state
                .library
                .entries()
                .iter()
                .map(|e| e.gameset.games.len())
                .sum();
            ...
            ui.horizontal_wrapped(|ui| {
                stat_card(
                    ui,
                    state.t(Key::DashboardPlatforms),
                    &platform_count.to_string(),
                );
                stat_card(
                    ui,
                    state.t(Key::DashboardDatFilesLoaded),
                    &dat_count.to_string(),
                );
                stat_card(
                    ui,
                    state.t(Key::DashboardGamesTracked),
                    &games_total.to_string(),
                );
