...
fn shaders_root() -> PathBuf {
    retrotools_common::config::plugin_data_dir_path("shaders").unwrap_or_else(|_| PathBuf::from("shaders-data"))
}
...
pub fn import_preset(dir: &Path, filename: &str, content: &str) -> Result<PathBuf, String> {
    ...
    if ext != "glslp" && ext != "slangp" {
        return Err(format!("'{filename}' is not a .glslp or .slangp preset file"));
    }
...
pub fn list_library(dir: &Path) -> std::io::Result<Vec<PathBuf>> {
    ...
    let mut paths: Vec<PathBuf> = std::fs::read_dir(dir)?
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .filter(|p| matches!(p.extension().and_then(|e| e.to_str()), Some("glslp") | Some("slangp")))
        .collect();
    paths.sort();
    Ok(paths)
}
...
pub fn override_relative_path(assoc: &ShaderAssociation) -> PathBuf {
    match assoc.scope {
        ShaderScope::System => PathBuf::from(&assoc.core_name).join(format!("{}.cfg", assoc.core_name)),
...
const GENERATED_MARKER: &str = "# Generated by Retro Tools 2026 -- safe to delete, will be regenerated";
...
impl Plugin for ShaderOverridesPlugin {
    ...
    fn run(&self, ctx: &PluginContext) -> PluginResult<PluginOutcome> {
        ...
        if associations.is_empty() {
            return Err("no shader associations saved yet — add one in Settings/Shaders first".into());
        }
        ...
        if ctx.dry_run {
            let mut summary = format!("[dry run] would generate {} shader override(s) in '{}'", plan.len(), ctx.output_dir.display());
            if !missing_presets.is_empty() {
                summary.push_str(&format!("; missing preset(s): {}", missing_presets.join(", ")));
            }
        ...
        for (assoc, preset_path) in &plan {
            ...
            std::fs::copy(preset_path, &copied_shader).map_err(|e| format!("cannot copy preset '{}': {e}", preset_path.display()))?;
            ...
            std::fs::write(&override_path, override_content(&copied_shader)).map_err(|e| e.to_string())?;
        ...
        let mut summary = format!("generated {} shader override(s) in '{}'", plan.len(), ctx.output_dir.display());
        if !missing_presets.is_empty() {
            summary.push_str(&format!(" — skipped association(s) with a missing preset: {}", missing_presets.join(", ")));
        }
...
impl Plugin for ShaderCleanupPlugin {
    ...
    fn run(&self, ctx: &PluginContext) -> PluginResult<PluginOutcome> {
        ...
        if ctx.dry_run {
            return Ok(PluginOutcome {
                summary: format!("[dry run] would remove {} generated file(s)", manifest.len()),
                files_written: Vec::new(),
            });
        }
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("rt26-plugin-shaders-test-{name}-{}", std::process::id()));
    ...
    fn override_paths_match_retroarch_layout() {
        ...
        assert_eq!(override_relative_path(&system), PathBuf::from("snes9x").join("snes9x.cfg"));
        ...
        assert_eq!(override_relative_path(&game), PathBuf::from("snes9x").join("SNES").join("Super Metroid.cfg"));
    }
    ...
    fn cleanup_removes_only_manifested_files_and_leaves_hand_made_ones() {
        ...
        std::fs::write(&generated, override_content(Path::new("shaders/crt-geom.slangp"))).unwrap();
    ...
    fn export_plugin_run_writes_override_and_manifest_end_to_end() {
        ...
        let gs = empty_gameset();
        let outcome = ShaderOverridesPlugin.run(&ctx(&gs, &output, false)).unwrap();
