...
/// with a linearly increasing backoff. Same design as
/// `retrotools-plugin-scraper`'s `RateLimiter` (duplicated rather than
/// shared across plugin crates — small and self-contained enough that a
/// shared dependency isn't worth the coupling). The sleep function is
/// injectable so tests can verify call counts/timing without a real
/// multi-second test.
